...
module Users = {
            ...
            switch(isValid){
              ...
              | Belt.Result.Ok(bool) => 
                switch(bool){
                  ...
                  let jwt = JsonWebToken.sign(~secret=`string("process.env.SECRET_TOKEN"), ~options, `json(myUser));
  ...
  let verify = token => {
    let bool = JsonWebToken.verify(token,`string("process.env.SECRET_TOKEN")) |> Belt.Result.isOk
    Js.Promise.(
